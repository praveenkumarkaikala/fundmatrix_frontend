  ...
  return (
    <div className="section-gap">
      <Card title="Expense-Ratio Compliance" hint="Charged rate (sum of latest annualised rate per expense type) vs the scheme TER limit"
        ...
        {c.status === 'BREACH' && <div className="alert alert-error" style={{ marginTop: 12 }}>Charged expense rate exceeds the scheme's expense-ratio limit.</div>}
        {c.status === 'WARN' && <div className="alert alert-warn" style={{ marginTop: 12 }}>Approaching the expense-ratio limit (≥ 80% utilised).</div>}
      </Card>
    </div>
  )
}
  ...
  return (
    <Modal title="Reverse Accrual" onClose={onClose}
      ...
      <div className="field" style={{ marginTop: 10 }}>
        ...
        <textarea rows={3} value={reason} onChange={(e) => setReason(e.target.value)} placeholder="e.g. Booked in error / duplicate entry" />
      </div>
    </Modal>
  )
}
    ...
    <Modal title="Book Expense Accrual" onClose={onClose}
      ...
      <form id="acc-form" onSubmit={submit}>
        ...
        <div className="form-row">
          <div className="field">
            <label>Annualised Rate (%)</label>
            <input type="number" step="0.0001" value={form.annualisedRate} onChange={(e) => setForm({ ...form, annualisedRate: e.target.value })} required />
          </div>
          <div className="field">
        ...
        <div className="field">
          <label>Amount (₹) — leave blank to auto-compute from AUM</label>
          <input type="number" step="0.01" value={form.accrualAmount} onChange={(e) => setForm({ ...form, accrualAmount: e.target.value })} placeholder="Auto = AUM × rate ÷ 365" />
        </div>
      </form>
    </Modal>
  )
